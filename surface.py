...
class PlaneWaves(object):
    ...
    def get_bed_depth(self):
        depth = [None] * (self._size[0] * self._size[1])
        i = 0.0
        for dep in range(0, int(self._size[0] / 2)):
            for dp in range(0, self._size[1]):
                depth[dep * self._size[0] + dp] = i;
            i += 0.01
            if i > 1:
                i = 1

        for dep in range(int(self._size[0] / 2), self._size[0]):
            for dp in range(0, self._size[1]):
                depth[dep * self._size[0] + dp] = i;
            i -= 0.01
            if i < 0:
