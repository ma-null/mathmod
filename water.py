...
void main() {
    ...
    float v_refl = v_reflectance;
    float v_rev_refl = 1 - v_refl;
    float usm = u_sky_mult;
    vec3 sc = sky_color;
    vec3 bc = bed_color;
    if (v_refl > 1) {
        v_refl = 1 - v_refl;
        v_rev_refl = -(v_rev_refl-1) * usm;
        ...
        diffused_intensity=0.0;
        reflected_intensity=0.0;
    }

    vec3 image_color=u_bed_mult*bc*v_mask + u_depth_mult*ambient_water*(1-v_mask);

    vec3 rgb = usm*sc*v_refl + image_color*v_rev_refl + diffused_intensity*u_sun_diffused_color + reflected_intensity*u_sun_reflected_color;
    
    sky_color = sky_color / 1.5;

    if (is_updown == 0) {
        image_color=u_sky_mult*sc*v_mask + u_depth_mult*ambient_water*(1-v_mask);
        rgb = bed_color*diffused_intensity + bed_color*reflected_intensity + image_color*v_rev_refl + usm*bc*v_refl; 
...
class Canvas(app.Canvas):
    ...
    def __init__(self, surface, sky="fluffy_clouds.png", bed="seabed.png"):
        ...
        self.program["u_bed_depth"]=self.surface.get_bed_depth();
...
if __name__ == '__main__':
    ...
    surface=CircularWaves(size=(230,230), max_height=0.03)
    c = Canvas(surface)
    c.measure_fps()
    app.run()
